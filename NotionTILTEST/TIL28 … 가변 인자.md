...
가변 개수(variable number)는 말 그대로 개수가 고정되지 않고, 호출할 때마다 달라질 수 있는 값의 개수를 말한다.

Object... args 는 가변 개수의 인자를 받을 수 있는 문법이다.
메서드 호출 시 전달된 모든 인자들은 컴파일러가 자동으로 배열(Object[])로 변환하여 메서드에 전달한다.
